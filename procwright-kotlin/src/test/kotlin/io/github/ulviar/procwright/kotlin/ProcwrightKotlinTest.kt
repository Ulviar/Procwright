...
class ProcwrightKotlinTest {
    ...
    @Test
    fun `kotlin duration overloads configure command timeout`() {
        val result =
            javaService().runCommand {
                args("-version")
                timeout(2.seconds)
            }
    ...
    @Test
    fun `pooled line session DSL configures worker and pool policies`() {
        javaService()
            .pooledLineSession {
                worker {
                ...
                maxSize(2)
                minIdle(0)
                acquireTimeout(500.milliseconds)
                maxRequestsPerWorker(10)
            }
            .use { pool ->
                val response = pool.request("hello")
    ...
    @Test
    fun `protocol adapter DSL configures typed request response protocol`() {
        val adapter =
            protocolAdapter<String, String> {
        ...
        javaService()
            ...
            .open()
            .use { session -> assertEquals("response:ping", session.request("ping")) }
    }

    @Test
    fun `protocol adapter DSL requires both handlers`() {
    ...
    fun `test methods keep junit compatible void return type`() {
        val testMethods =
            ProcwrightKotlinTest::class.java.declaredMethods.filter { method ->
