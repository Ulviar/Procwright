...
- Do not read raw `Session.stdout()` or `Session.stderr()` after handing the session to `Expect`, `lineSession`, or
  `listen`-style helpers.
...
request/response line protocols.
