...
public final class Expect implements AutoCloseable {
    ...
    Expect(Session session, ExpectOptions options) {
     ...
     */
    public static Expect on(Session session) {
     ...
     * @return expect helper
     */
    public static Expect on(Session session, ExpectOptions options) {
        return new Expect(session, options);
    }
    ...
    public Expect send(String text) {
        ...
        try {
            session.send(text);
            transcript.appendAction("send: " + printable(text));
            return this;
    ...
    public Expect sendLine(String line) {
        ...
        try {
            ...
            transcript.appendAction("send line: " + printable(line));
            return this;
        } catch (RuntimeException exception) {
    ...
    public Expect expectText(String text, Duration timeout) {
        Objects.requireNonNull(text, "text");
        transcript.appendAction("expect text: " + printable(text));
    ...
    public Expect expectRegex(Pattern pattern, Duration timeout) {
        ...
        transcript.appendAction("expect regex: " + printable(pattern.pattern()));
        long deadlineNanos = deadlineFromNow(requirePositive(timeout, "timeout"));
        synchronized (monitor) {
            while (true) {
                throwIfFailed();
