package io.github.ulviar.icli;

import static org.junit.jupiter.api.Assertions.assertInstanceOf;

import io.github.ulviar.icli.command.CommandException;
import io.github.ulviar.icli.command.CommandExecutionException;
import io.github.ulviar.icli.command.CommandResult;
import io.github.ulviar.icli.session.ExpectException;
import io.github.ulviar.icli.session.LineSessionException;
import io.github.ulviar.icli.session.LineTranscript;
import io.github.ulviar.icli.session.PooledLineSessionException;
import io.github.ulviar.icli.session.PooledProtocolSessionException;
import io.github.ulviar.icli.session.ProtocolSessionException;
import io.github.ulviar.icli.session.ProtocolTranscript;
import io.github.ulviar.icli.session.StreamException;
import io.github.ulviar.icli.session.StreamTranscript;
import org.junit.jupiter.api.Test;

final class IcliExceptionTest {

    @Test
    void coreFailureTypesShareCommonBaseException() {
        assertInstanceOf(IcliException.class, new CommandException(new CommandResult(1, "", "")));
        assertInstanceOf(IcliException.class, new CommandExecutionException("failed"));
        assertInstanceOf(
                IcliException.class,
                new LineSessionException(LineSessionException.Reason.TIMEOUT, new LineTranscript("", false), "failed"));
        assertInstanceOf(
                IcliException.class,
                new ProtocolSessionException(
                        ProtocolSessionException.Reason.TIMEOUT,
                        new ProtocolTranscript("", false, false, false),
                        "failed"));
        assertInstanceOf(
                IcliException.class,
                new PooledLineSessionException(PooledLineSessionException.Reason.CLOSED, "failed"));
        assertInstanceOf(
                IcliException.class,
                new PooledProtocolSessionException(PooledProtocolSessionException.Reason.CLOSED, "failed"));
        assertInstanceOf(
                IcliException.class,
                new ExpectException(ExpectException.Reason.TIMEOUT, new LineTranscript("", false), "failed"));
        assertInstanceOf(IcliException.class, new StreamException("failed", new StreamTranscript("", false), null));
    }
}
