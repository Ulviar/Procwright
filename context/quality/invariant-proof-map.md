...
| Public API не раскрывает internal/external types и не растет без решения. | JPMS descriptors, exact signature baseline, package/external boundary checks. | `PublicApiSurfaceTest`, `PackageBoundaryTest`, `ExternalLibraryBoundaryTest`, `apiCompatibilityCheck`. |
