...
| Pooled protocol sessions reuse existing protocol-session runtime and keep adapter state worker-owned. | `CommandService.pooledProtocol`, serialized per-worker adapter factory, `PooledProtocolSessionInvocation`, `DefaultPooledProtocolSession`, `SessionScenarioSupport`. | `ProtocolSessionIntegrationTest`, `CommandServiceApiExamples.pooledProtocolSessionScenario`. |
