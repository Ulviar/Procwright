...
- migration notes не создаются для первого публичного релиза без пользователей.
...
закрыт отдельным ADR, а тяжелые benchmarks остаются отдельной будущей фазой.
