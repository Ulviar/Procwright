...
пользовательские пакеты, а `io.github.ulviar.icli.internal` и вложенные runtime-пакеты остаются закрытыми.
...
- `io.github.ulviar.icli.session` содержит публичные sealed handle interfaces и immutable value/option/exception types;
- `io.github.ulviar.icli.internal.session` содержит stateful implementations и runtime factories:
...
- `module-info.java` экспортирует только public API packages:
  `io.github.ulviar.icli`, `command`, `diagnostics`, `preset`, `session`, `terminal`;
- `internal` и `internal.session` не экспортируются.

Diagnostics dispatcher и diagnostic schema validator остаются в `io.github.ulviar.icli.internal`; публичный
...
- core artifact стал именованным Java module `io.github.ulviar.icli`;
