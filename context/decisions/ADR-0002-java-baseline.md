...
Заменен [ADR-0004](ADR-0004-java-25-baseline.md).

## Контекст

Для iCLI важно выбрать baseline, который поддерживает современные конструкции Java, но не сужает потенциальную
аудиторию без необходимости.
...
Сборка должна компилировать с `--release 21`, даже если локальный JDK новее.
...
Это решение было пересмотрено после смены приоритета проекта: iCLI рассматривается прежде всего как проект создания
максимально качественного современного кода, а не как практически наиболее совместимая библиотека. Новый baseline
зафиксирован в [ADR-0004](ADR-0004-java-25-baseline.md).
