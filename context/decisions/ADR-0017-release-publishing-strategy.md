...
- `workflowSecurityCheck` принимает fail-closed canonical subset для security-полей workflow и проверяет pinned action
  SHA, явные минимальные permissions, allowlist protected environments для secrets/write jobs, изоляцию target
  checkout, immutable artifact ID/digest bindings и verification до Central/Pages deployment. `bash -n` отдельно
  проверяет syntax fixed scripts. Реальное выполнение workflow остается behavioral proof.
