...
Результат comparison harness и qualitative assessment подтверждают исходную гипотезу: простые one-shot сценарии
закрываются разными библиотеками, но реальные CLI-workflow быстро требуют набора инвариантов, которые не должны вручную
собираться пользователем из низкоуровневых stream/process primitives.

## Решение

iCLI остается scenario-first библиотекой, а не thin wrapper над `ProcessBuilder` или фасадом над одной внешней
process-библиотекой.
...
### Сделать iCLI thin wrapper над `ProcessBuilder`
...
Отклонено, потому что каждая библиотека сильна в своем scope, но ни одна не задает полный scenario-first contract iCLI.
...
Отклонено, потому что это превратило бы iCLI в каталог adapters и размывало бы единый пользовательский язык сценариев.

## Последствия

Плюсы:

- iCLI сохраняет собственную архитектурную идентичность.
...
- `:icli-comparison:comparisonReport` регулярно проверяет scenario coverage внешних библиотек.
- `:icli-comparison:comparisonCheck` выполняет non-mutating regression gate и пишет отчет в `build/`.
- [../comparison/qualitative-assessment.md](../comparison/qualitative-assessment.md) фиксирует API/task-fit/maintenance
  assessment, чтобы ADR не опирался только на quantitative report.
