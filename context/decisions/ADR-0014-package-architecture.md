...
- `PublicApiSurfaceTest` блокирует преждевременный `module-info.java`, пока runtime-only public session support остается
  внутри экспортируемого `session` package.
