...
Имя `CommandService` сохраняется для первого release candidate. Оно переносит сильную идею старой версии: пользователь
создает сервис вокруг базовой команды, а затем выбирает сценарий. Альтернативы вроде `CommandExecutor` хуже отражают
sessions, streaming, pooling и structured integrations.
