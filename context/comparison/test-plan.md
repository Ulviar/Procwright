...
`comparisonReport` не является JMH benchmark. Он проверяет прикладные workflow и failure modes. Machine-dependent
latency используется только как один сигнал, а не как абсолютная оценка качества.

JMH performance research вынесен в отдельный source set и описан в [jmh-benchmarks.md](jmh-benchmarks.md).
