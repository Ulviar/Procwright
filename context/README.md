...
Эта папка заменяет старую большую базу задач компактным набором источников правды для текущего clean rewrite iCLI.
...
Старый проект остается в Git history до переписывания `main`. Его документы и код можно использовать как reference, но
они не имеют авторитета в текущем clean rewrite, если новое решение явно не переносит идею.
