...
Scenario API baseline первого RC включает `run`, `interactive`, `lineSession`, `protocolSession`, `pooled` и
`pooledProtocol`. Generic/core async request API и raw session pooling не входят в текущую baseline; узкий cancellable
JSON Lines helper остается частью optional integrations module.
